...
| **2** | **`Clean`** | Cleaned and filtered data subset retaining compliant properties ($\text{GFA} \ge 50,000\text{ sq. ft.}$). |
...
| **8** | **`Factors`** | Official municipal carbon emission coefficients ($\text{kg CO}_2\text{e / kBtu}$) across electricity, natural gas, fuel oil, and steam. |
...
Across all financial sheets, statutory penalty exposure is evaluated using:
$$\text{Penalty} = \text{Total Emissions} \times \$268$$
where **$268** is the mandatory fine per metric ton of $\text{CO}_2\text{e}$ exceeding statutory thresholds under NYC Local Law 97.
